#!/usr/bin/env node
/**
 * Generate public/mpairwe-lauben-mini-cv.pdf from /minicv (source of truth).
 *
 * One-page, hackathon-tailored brief C.V.
 *
 * Flow:
 *   1. Spawn `next start` on port 3030 (uses existing .next build)
 *      Falls back to `next dev` if no build is found.
 *   2. Poll http://localhost:3030/minicv until it returns 200.
...
const PORT = 3030
const URL = `http://localhost:${PORT}/minicv`
const OUTPUT = resolve("public/mpairwe-lauben-mini-cv.pdf")
...
async function printPdf({ engine, bin }) {
  if (engine === "chrome") {
    console.log(`[minicv-pdf] engine: chrome (${bin})`)
    return run(bin, [
      "--headless=new",
      "--no-sandbox",
      "--disable-gpu",
  ...
  if (engine === "weasyprint") {
    console.log(`[minicv-pdf] engine: weasyprint (${bin})`)
    // WeasyPrint fetches the URL + its linked stylesheets and renders the
    // print stylesheet (no JS — the page is server-rendered, so that's fine).
...
async function main() {
  ...
  console.log(`[minicv-pdf] Starting next ${mode} on :${PORT}`)
  ...
  try {
    ...
    console.log(`[minicv-pdf] /minicv responding; printing to ${OUTPUT}`)
    await printPdf(engine)
    console.log(`[minicv-pdf] ✓ PDF written to ${OUTPUT}`)
  } finally {
    server.kill("SIGTERM")
    await sleep(500)
...
main().catch((err) => {
  console.error("[minicv-pdf] failed:", err.message)
  process.exit(1)
})
